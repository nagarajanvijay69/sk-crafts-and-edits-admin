               ...
               <div className="w-[90dvw] mx-auto">
                    <div className="">
                         ...
                         <div className="img mt-5 flex gap-3 flex-col flex-wrap lg:w-lg">
                              <p>Image</p>
                              <label htmlFor='category-img' className="add my-5 bg-dark h-30">
                                   ...
                                   <input type="file" onChange={(e) => {
                                        ...
                                        setSrc(url);
                                   }} accept="image/*" id='category-img' className='h-0 w-0 bg-dark' />
                              </label>
                         </div>
                         <button className="mt-5 bg-gray-800 text-white font-medium rounded cursor-pointer flex items-center justify-center h-10 w-25" onClick={handleSubmit}>
                              {
                                   load ?
                                        < Load />
                                        : "ADD"
                    ...
                    <ul className='flex flex-col gap-5 my-6'>
                         {
                              categorys.map((item, index) => (
                                   <li key={index} className='pl-5 flex gap-7 border-2 border-gray-500 items-center lg:w-lg'><img src={`${item.image}`} className='h-25 w-25 m-2' />{item.name}</li>
                              ))
                         }
                    </ul>
